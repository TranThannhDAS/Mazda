...
namespace Mazda_Api.Controllers
{
    ...
    public class CategoryController : BaseController
    {
        ...
        {
            var CategoryType  = DataContext.CategoryTypes.Where(p => p.Name.Equals(categoryType)).FirstOrDefault();
            var categoryDTOList = new List<CategoryDto>();
            var categoryList = DataContext.Categories.Where( p => p.CategoryTypeId == CategoryType.Id).ToList();         
        ...
        {
            var Category = DataContext.Categories.FirstOrDefault(p=> p.Id == categoryid);
            return Category.Name;
        }
        [ApiExplorerSettings(IgnoreApi = true)]
        public async Task<int> GetCategoryId(string name)
        {
            var Category = DataContext.Categories.FirstOrDefault(p => p.Name.Equals(name));
            return Category.Id;
        }
        //[ApiExplorerSettings(IgnoreApi = true)]
        //public async Task<string> GetCategoryType(int categoryId)
